...
## AVANCEMENT — AUD-301..390 (session 2026-08-05, audit + correctifs pytest REEL)

> Methode : audit contre l'arbre COMPLET (3201 fichiers), correctifs test-first verifies pytest REEL
> en clone cloud (44 tests neufs verts), portes byte-identiques (tar SHA-256 + py_compile) et commites
> en LOCAL. Sans multi-agent. Aucun faux commit, aucune mesure inventee. 0 reseau, 0 ordre reel, 0 cle
> privee, enveloppe 1000 USD. Cette tranche (data-engineering) est entierement offline-prouvable :
> AUCUN item BLOCKED_EXTERNAL (les connecteurs live etaient tous en 261-300).

### FIXED cette session — 49 items, nouveau code + tests (RED->GREEN, 44 tests verts)
- **Lot 1 — SHA `4e7e2d32` (22 tests)** : gardes ANTI-MENSONGE donnees (384 no-success-si-zero-data, 387 no-zeros-inventes, 388 no-carry-forward-silencieux, 350 reject-revised-non-PiT, 386 quarantaine champs inconnus) ; fiabilite flux (318 liveness!=progression, 319 last-useful-ts, 322 stale par stream, 323 DLQ, 324 migrations schema, 385 compteur events utiles) ; stockage/lineage (301 partition Parquet, 367 hash shard, 365 lineage ligne) ; gouvernance sources (311 registre BLOCKED_EXTERNAL, 351 cache-paye TTL, 352 low-latency policy, 373 pin endpoints, 374 licences/quotas/couts, 375 cle read-only, 376 conformite, 377 dashboard sante, 378 SLA, 379 onboarding, 380 retrait).
- **Lot 2 — SHA `691b7ff3` (22 tests)** : integrite wallets (354 sybils/miroirs, 355 transferts!=PnL, 356 survivorship, 357 wallets liquides, 362 consensus pondere-independance, 353 seuil confiance merge) ; qualite marche (308 regime options, 360 filtre manipulation, 361 pannes correlees, 372 changement API silencieux) ; normalisation unites (305/345 OI notionnel, 307/346 sens liquidation, 349 mark/index versionne, 341 symbol master PiT) ; integrite code/pipeline (310 courbe apprentissage OOS, 312 modules sans appelant, 314 except larges, 316 derive tasklist, 369 adaptateur unique live/replay, 381 collecteurs doublons, 382 correspondance registre/lanceur/superviseur, 390 ressources par source).

### VERIFIED_DONE (41) — impl deja presente, verifiee (fichier:symbole)
- **302** data_mesh.lineage + storage_partition.lineage_ligne · **303** backtest/{book_replay,ledger_replay_v9,replay_engine,trade_tick_replay} + ops/market_truth_replay · **304** provenance_label asof (PiT) · **306/344** funding/funding_times.py · **309** copying/consensus_leader_selector + following/entity_consensus · **313** interdire_success + AUD-263 · **315** unification legacy/canonique (AUD-028/029) · **317** unification registres collecteurs (AUD-050) · **320** venue_readiness + wallet scoring · **321** data_mesh statuts REQUIRED/OPTIONAL/DEGRADED.
- **325** realtime/durable_dedup + copy_vault/exactly_once_source_fill · **326** suite dydx_v4 (snapshots ms) · **327** accounting/fixed_point_core.py (Decimal) · **328/329** executemany + PRAGMA (scenario_db, l2_snapshot_cache, trades_recorder, connectors/base) · **330** capture/atomic_checkpoint + copy_vault/epoch_local_checkpoint · **331** collection/stockage_brut_borne · **332** copy_vault/per_vault_queue_cap + api_governance/request_qos · **333** api_governance/request_qos (QoS/load-shed) · **334** ops/quota_stockage · **335** api_governance/weighted_rate_limiter + hyperliquid/rate_limits · **336** circuit_breaker (collection/core/risk/copying).
