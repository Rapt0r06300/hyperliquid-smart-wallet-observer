/* HyperSmart smooth metagraph renderer
 * READ-ONLY UI layer: it never changes paper PnL, it only animates display
 * between real server samples to remove spike/pillar movement.
 */
(function () {
  const state = {
    samples: [],
  ...
  function addSample(pnl, ts, source, equity) {
    ...
    if (last && Math.abs(last.value - value) < 0.000001 && at - last.ts < 800) return;
    state.samples.push({ value, ts: at, source: source || "sample", equity: num(equity, 1000) });
    ...
    if (!Number.isFinite(state.displayPnl)) state.displayPnl = value;
  }
  ...
  function buildDisplaySeries() {
    ...
    for (let i = 0; i < raw.length - 1; i += 1) {
      const a = raw[i];
      const b = raw[i + 1];
      series.push({ ...a, display: a.value, anchor: true });
      const dt = Math.max(1, b.ts - a.ts);
      const steps = clamp(Math.ceil(dt / 900), 1, 10);
      for (let s = 1; s < steps; s += 1) {
        ...
        series.push({
          ...
          source: "ui_interpolation",
          equity: a.equity + (b.equity - a.equity) * eased,
          anchor: false,
        });
      }
    }
    series.push({ ...raw[raw.length - 1], display: raw[raw.length - 1].value, anchor: true });
    return series.slice(-260);
  }
  ...
  function drawSmoothMetagraph() {
    ...
    const damping = 0.075;
    state.displayPnl += (state.targetPnl - state.displayPnl) * damping;
    if (Math.abs(state.displayPnl - state.targetPnl) < 0.002) state.displayPnl = state.targetPnl;
    const base = buildDisplaySeries();
    const now = Date.now();
    ...
    // Faint raw anchors: truthful samples.
    ctx.globalAlpha = 0.26;
    ctx.fillStyle = "#e8faff";
    series.forEach((p, i) => {
      if (!p.anchor) return;
      ctx.beginPath(); ctx.arc(xFor(i), yFor(p.display), 2.2, 0, Math.PI * 2); ctx.fill();
    });
    ctx.globalAlpha = 1;
    ...
    gradient.addColorStop(0.55, "rgba(0,217,255,0.07)");
    gradient.addColorStop(1, "rgba(255,59,95,0.10)");
    ctx.beginPath();
    series.forEach((p, i) => {
      const x = xFor(i), y = yFor(p.display);
      if (i === 0) ctx.moveTo(x, y);
      else {
        const prevX = xFor(i - 1), prevY = yFor(series[i - 1].display);
    ...
    const lastY = yFor(state.displayPnl);
    ctx.fillStyle = positive ? "#00ff88" : "#ff3b5f";
    ctx.beginPath(); ctx.arc(lastX, lastY, 4.2, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "rgba(232,250,255,0.65)"; ctx.stroke();

    ctx.fillStyle = "#8aa0b6";
    ctx.font = "12px Cascadia Code, Consolas, monospace";
    ctx.fillText(formatUsdLocal(maxValue), 8, plotTop + 10);
    ctx.fillText(formatUsdLocal(minValue), 8, plotBottom);
    const ageMs = Math.max(0, Date.now() - state.lastTickAt);
    badge.textContent = `Temps reel fluide ${formatUsdLocal(state.displayPnl)}${ageMs > 5000 ? " · tick ancien" : ""}`;
    ...
    canvas.onmousemove = (event) => {
      ...
      tooltip.innerHTML = `<strong>Metagraphe temps reel</strong><br>Gain/perte affiche: ${formatUsdLocal(p.display)}<br>Source: ${p.source || "sample"}<br>${p.anchor ? "Point reel serveur" : "Interpolation visuelle entre points reels"}`;
    };
  ...
  window.drawSimulationMetaGraph = function smoothDraw(candles, equity) {
    ...
    if (!state.raf) state.raf = window.requestAnimationFrame(animationLoop);
    drawSmoothMetagraph();
    return undefined;
  };
