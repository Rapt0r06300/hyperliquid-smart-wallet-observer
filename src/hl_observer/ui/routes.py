...
def create_router(settings: Settings, state: UiState, bus: UiEventBus) -> APIRouter:
    ...
    def build_bot_simulation(
        ...
        *,
        ...
        """Professional local simulation using the UnifiedDecisionEngine."""
        cost_bps = 12.0
        curr_ms = now_timestamp_ms or now_ms()

        cfg = SimulationConfig(
            risk_config=RealtimeCopyRiskConfig(
                min_edge_required_bps=settings.risk.min_edge_required_bps,
                fee_bps=4.0, spread_bps=3.0, slippage_bps=5.0,
                max_signal_age_ms=10 * 60_000, starting_equity_usdt=starting_equity_usdt,
        ...
        state_obj = SimulationState(
            virtual_positions=dict(existing_positions or {}),
            ledger_events=[dict(row) for row in (existing_events or []) if isinstance(row, dict)],
            processed_delta_keys=set(processed_delta_keys or set()),
            equity_usdt=starting_equity_usdt, starting_equity_usdt=starting_equity_usdt
        )
        ...
        for row in chronological:
            dk = engine._delta_key(row)
            if dk in state_obj.processed_delta_keys: continue
            state_obj.processed_delta_keys.add(dk)
            engine.process_delta(row, curr_ms, mids, state_obj, chronological)

        # Output mapping for UI compatibility
        ledger = state_obj.ledger_events[-max_events:]
        open_pos: list[dict[str, Any]] = []
        unrealized = 0.0
        persisted: dict[str, dict[str, Any]] = {}

        for key, pos in state_obj.virtual_positions.items():
            _, coin, direction = key.split("|")
            px_mark = mids.get(coin.upper()) or pos["avg_price"]
            gross = (px_mark - pos["avg_price"]) * pos["size"] if direction == "LONG" else (pos["avg_price"] - px_mark) * pos["size"]
            cost_exit = abs(pos["size"] * px_mark) * cost_bps / 10000.0
            net_unreal = gross - cost_exit
            unrealized += net_unreal

            open_pos.append({
                "wallet_address": key.split("|")[0], "coin": coin, "direction": direction,
                "size": round(pos["size"], 10), "avg_entry_price": round(pos["avg_price"], 8),
                "mark_price": round(px_mark, 8), "entry_costs_remaining": round(pos["entry_costs"], 6),
                "unrealized_pnl_usdc": round(net_unreal, 6), "research_only": True,
            })
            persisted[key] = pos

        open_pos.sort(key=lambda x: abs(float(x["unrealized_pnl_usdc"])), reverse=True)
        real_pnl = sum(float(r.get("estimated_net_pnl_usdc") or 0.0) for r in ledger if r.get("status") == "LOCAL_REPLAY")
        tot_pnl = real_pnl + unrealized

        return {
            "events": list(reversed(ledger[-240:])),
            "ledger_events": ledger,
            "processed_delta_keys": list(state_obj.processed_delta_keys)[-10000:],
            "virtual_positions_state": persisted,
            ...
            "estimated_net_pnl_usdc": round(tot_pnl, 6),
            "entry_costs_paid_usdc": round(sum(float(r.get("fee_cost_usdc") or 0.0) for r in ledger if "ENTRY" in r.get("bot_replay_action", "") or "ADD" in r.get("bot_replay_action", "")), 6),
            "exit_costs_paid_usdc": round(sum(float(r.get("fee_cost_usdc") or 0.0) for r in ledger if "CLOSE" in r.get("bot_replay_action", "") or "REDUCE" in r.get("bot_replay_action", "")), 6),
            "total_costs_paid_usdc": round(state_obj.total_fees_usdc, 6),
            ...
            "magic_profile": {
                ...
                "consensus_window_seconds": cfg.consensus_window_ms // 1000,
