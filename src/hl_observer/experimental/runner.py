...
def _etat_leader(pos: dict, root: Path, *, now_ms: float) -> dict:
    ...
    out = {"action": "AUCUN", "motif": None, "entry_szi": entry_szi, "current_szi": entry_szi,
           "snapshot_ts": None, "snapshot_id": None}
    if not vault or not coin or abs(entry_szi) <= 0:
        out["motif"] = "PAS_DE_REFERENCE_LEADER"; return out
    try:
        lignes = (root / "runtime" / "data" / "vault_snapshots.jsonl").read_text(
            encoding="utf-8", errors="ignore").splitlines()
    except OSError:
        out["motif"] = "PAS_DE_SNAPSHOTS"; return out
    dernier, brut = None, ""
    for l in reversed(lignes[-8000:]):
        ...
        if d.get("vault") == vault:
            ...
            break
    if not dernier:
        out["motif"] = "AUCUN_SNAPSHOT_VAULT"; return out
    ...
    ok, motif = EP.snapshot_complet_ok({"complet": complet, "ts_ms": ts}, coin,
                                       ts_entree_ms=float(pos.get("ts_ouverture_ms") or 0.0), now_ms=now_ms,
                                       fraicheur_max_ms=SNAPSHOT_FRAICHEUR_MAX_MS)
    if not ok:
        out["motif"] = motif; return out
    # P1 : déduplication — un snapshot déjà consommé ne redéclenche jamais d'action.
    if pos.get("last_vault_snapshot_id") == snap_id:
        out["motif"] = "SNAPSHOT_DEJA_CONSOMME"; return out
    ...
    for p in (dernier.get("positions") or []):
        if str(p.get("coin") or "").upper() == coin:
            cur = float(p.get("szi") or 0.0); break
    out["current_szi"] = cur
    last_applied = float(pos.get("last_leader_szi_applied") if pos.get("last_leader_szi_applied") is not None else entry_szi)
    out["action"] = EP.classifier_changement_leader(entry_szi=entry_szi, current_szi=cur, last_applied_szi=last_applied)
    return out
    ...
    for pos in list(store["ouvertes"].values()):
        ...
        if pos.get("type_pnl") == "dislocation":              # cross-venue COURT TERME : capture/stop rapide
            ...
            if not raison and not car:
                ...
                if dm["action"] == "SORTIE":
                    ...
                    fermetures.append(MP.sortir(pos, store, root, prix_sortie=dm["mark_conservateur"],
                                                cout_sortie_bps=cout_dm, base_courant_bps=gap_ent,
            ...
            if raison:
                ...
                if jambes:
                    fermetures.append(MP.sortir_deux_jambes(pos, store, root, jambes=jambes, raison=raison, now_ms=now_ms))
                else:
                    fermetures.append(MP.sortir(pos, store, root, prix_sortie=pos.get("prix_entree"),
            ...
            ferme_complet = horizon_atteint or action_leader in ("CLOSE", "FLIP_LONG_SHORT", "FLIP_SHORT_LONG")
            ...
            frais_ok = bool(ba) and (ba.get("ts_ms") is None or (now_ms - float(ba["ts_ms"])) <= BIDASK_FRAICHEUR_MAX_MS)
            px_exec = INV.prix_sortie_executable(int(pos.get("sens") or 1), bid=(ba or {}).get("bid"),
                                                 ask=(ba or {}).get("ask")) if (ba and frais_ok) else None
            ...
            if reduce_partiel:                                # P1+P4 : cible depuis le NOTIONNEL INITIAL, idempotent
                rp = EP.reduire_vers_cible(pos, entry_leader_szi=et["entry_szi"], current_leader_szi=et["current_szi"],
                ...
                if rp.get("action") == "REDUCE":
                    ...
                    fermetures.append(MP.reduire(
                        pos, store, root, notional_ferme_usd=rp["notional_ferme_usd"],
                        notional_residuel_usd=rp["notional_residuel_usd"], realized_usd=rp["realized_usd"],
                        prix_sortie=prix_sortie, cout_sortie_bps=cout_sortie, raison="LEADER_A_REDUIT", now_ms=now_ms,
                        entry_cost_allocated_usd=rp["entry_cost_allocated_usd"], exit_cost_usd=rp["exit_cost_usd"],
                        leader_szi_applied=et["current_szi"], snapshot_ts=et["snapshot_ts"], snapshot_id=et["snapshot_id"]))
            ...
            if ferme_complet:
                ...
                fermetures.append(MP.sortir(pos, store, root, prix_sortie=prix_sortie, cout_sortie_bps=cout_sortie,
                                            raison=raison, now_ms=now_ms))
    return fermetures
