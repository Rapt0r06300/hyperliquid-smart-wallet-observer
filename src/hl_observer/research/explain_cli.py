...
from hl_observer.research.ollama_advisor import advise_json_from_summary
from hl_observer.research.ollama_signal_rater import rate_signal_candidate
...
def build_explanations_from_events(events: list[dict], *, limit: int = 8) -> dict:
    ...
    for ev in seq:
        ...
        ai_rating = rate_signal_candidate({
            "coin": coin,
            "side": side,
            "decision_reason": reason,
            "edge_remaining_bps": ev.get("edge_remaining_bps"),
            "signal_age_ms": ev.get("signal_age_ms"),
            "consensus_wallets": ev.get("consensus_wallets"),
            "liquidity_score": ev.get("liquidity_score"),
            "copy_degradation_bps": ev.get("copy_degradation_bps"),
        }).to_dict()
