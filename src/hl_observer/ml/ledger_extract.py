...
__all__ = ["rows_outcomes_from_samples", "rows_outcomes_from_events", "ingest_snapshot_to_samples", "rows_outcomes_with_refusals"]


def rows_outcomes_with_refusals(
    events: list[dict],
    marks_by_coin: dict[str, list[tuple[float, float]]],
    *,
    accepted_context: str = "REPLAY",
    horizon_min: float = 10.0,
    cost_bps: float = 12.0,
):
    """Combine accepted OPEN->CLOSE pairs AND shadow-labeled refusals.

    This is the corpus-growth entry point: instead of learning only from the
    handful of trades we took, the model also learns from every measurable
    refusal (what it would have produced on real forward marks). Refused-shadow
    rows are tagged SHADOW_REFUSED so callers can weight or separate them.
    """

    from hl_observer.ml.refused_shadow_extract import rows_outcomes_from_refusals

    rows_a, out_a = rows_outcomes_from_events(events, context=accepted_context)
    rows_r, out_r = rows_outcomes_from_refusals(
        events, marks_by_coin, horizon_min=horizon_min, cost_bps=cost_bps
    )
    return rows_a + rows_r, out_a + out_r
