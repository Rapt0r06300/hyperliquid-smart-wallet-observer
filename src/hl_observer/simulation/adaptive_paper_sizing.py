...
def adaptive_paper_margin(
    *,
    ...
    if session_pnl <= -0.75:
        cap *= 0.65
        reason = "SESSION_DRAWDOWN_SIZE_REDUCED"
    if session_pnl <= -2.0:
        cap *= 0.35
        reason = "SESSION_DEEP_DRAWDOWN_SIZE_REDUCED"
    if losses >= 2:
        cap *= 0.75
    ...
    if losses >= 3 and session_pnl <= -1.0 and float(edge_remaining_bps or 0.0) < 75.0:
        return AdaptivePaperSize(
            requested_margin_usdt=round(requested, 6),
            final_margin_usdt=0.0,
            ...
            reason="ADAPTIVE_SIZE_BLOCKED_AFTER_LOSS_STREAK",
        )
    cap = max(0.0, cap)
