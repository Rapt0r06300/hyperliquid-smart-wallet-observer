...
def compute_shadow_signals(
    *,
    ...
    # --- V26 L1 — vetos funding/tendance d'edge (repo 32) : vue SHADOW read-only ---
    # L'écriture de l'historique d'edge vit dans le scorer (apply_v26_entry_vetos) ;
    # ici on EXPOSE seulement l'état courant. Vide/None = état honnête, jamais fabriqué.
    try:
        from hl_observer.funding.funding_runtime_cache import recent_rates
        from hl_observer.signals.v26_entry_vetos import edge_trend, funding_sanity

        out["shadow_edge_trend"] = edge_trend(coin, side)
        _fs = funding_sanity(recent_rates(coin))
        out["shadow_funding_ok"] = _fs.ok
        out["shadow_funding_code"] = _fs.code
        out["shadow_funding_z"] = _fs.z_score
    except Exception:
        out["shadow_edge_trend"] = None
        out["shadow_funding_ok"] = None
        out["shadow_funding_code"] = None
        out["shadow_funding_z"] = None
