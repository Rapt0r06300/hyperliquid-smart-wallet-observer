...
_SPEC = (
    ...
    ("FUNDING_SPIKE", "MARKET", Severity.BLOCK, True, "Funding anormal (spike vs distribution 24h).", "Attendre le retour du funding dans sa plage normale."),
    ("FUNDING_HISTORY_WARMUP", "MARKET", Severity.WARN, True, "Historique funding trop jeune.", "Attendre assez d'echantillons de funding."),
    ("EDGE_TRENDING_DOWN", "EDGE", Severity.BLOCK, True, "Edge net en decroissance sur les dernieres mesures.", "Attendre la stabilisation de l'edge."),
    ("STOPLOSS_GUARD_ACTIVE", "RISK", Severity.BLOCK, True, "Trop de stop-loss recents (garde freqtrade).", "Attendre la fin du halt stop-loss."),
    ("MARKET_LOW_PROFIT_BLOCKED", "MARKET", Severity.BLOCK, True, "Marche cumulativement perdant sur la fenetre.", "Blacklist temporaire du marche."),
    ("WINDOWED_DRAWDOWN_HALT", "RISK", Severity.BLOCK, True, "Drawdown fenetre depasse (protection).", "Pause le temps du cooldown."),
    ("GRADED_HALT_AMBER", "RISK", Severity.BLOCK, True, "Halt gradue AMBER (risque reduit).", "Nouvelles entrees limitees jusqu'au retour GREEN."),
    ("GRADED_HALT_RED", "RISK", Severity.BLOCK, True, "Halt gradue RED (arret cote).", "Sorties forcees paper puis cooldown."),
    ("COST_BUDGET_EXCEEDED", "EDGE", Severity.BLOCK, True, "Couts au-dela du budget du tier leader.", "Refuser (budget de couts par tier)."),
    ("MARKET_QUALITY_LOW", "MARKET", Severity.BLOCK, True, "Qualite de marche insuffisante (forager).", "Rester sur les top-K marches sains."),
