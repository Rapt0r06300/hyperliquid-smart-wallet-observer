...
class CopyInputs:
    ...
    # V26 L1 — vetos funding/tendance (None = inconnu = ne bloque jamais)
    funding_spike: bool | None = None
    funding_warmup: bool | None = None
    edge_trend_decreasing: bool | None = None
...
def evaluate_copy_candidate(inp: CopyInputs) -> CopyDecision:
    ...
    # 9) funding sain (V26 L1, repo 32) — None (inconnu) ne bloque jamais
    if inp.funding_spike is True:
        return block("FUNDING_SPIKE")
    if inp.funding_warmup is True:
        return block("FUNDING_HISTORY_WARMUP")
    passed.append("funding_sane")

    # 10) edge stable (V26 L1, repo 32) — None (pas assez d'historique) ne bloque jamais
    if inp.edge_trend_decreasing is True:
        return block("EDGE_TRENDING_DOWN")
    passed.append("edge_trend")
