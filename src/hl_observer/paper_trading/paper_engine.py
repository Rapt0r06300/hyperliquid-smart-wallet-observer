...
class PaperEngine:
    ...
    def mark_to_market(self, marks: dict[str, float]) -> tuple[float, float, float]:
        ...
        self.ledger.mark_to_market(_clean_marks_for_ledger(marks), timestamp_ms=int(time.time() * 1000))
...
def _ledger_refs(delta: LeaderDelta, *, exec_result: ExecResult, paper_position_id: str) -> dict[str, object]:
    return {
        ...
        "embedded_net_cost_bps": round(float(exec_result.net_cost_bps), 8),
        "embedded_slippage_bps": round(float(exec_result.slippage_bps), 8),
        "embedded_fee_bps": round(float(exec_result.fee_bps), 8),
