...
budgets SOMMES au-dela d'un master unique de 100 USD. Ce checker somme les budgets engages et
...
def verifier_enveloppe(budgets: Mapping[str, float], *, master_usd: float = ENVELOPPE_MASTER_USD) -> dict:
    ...
    budgets engages <= master. Un budget exploratoire ne peut donc pas s'ajouter par-dessus les 100."""
    ...
    return {"total_engage": total, "master": float(master_usd), "respecte": not depasse,
            ...
            "raison": ("OK" if not depasse
                       else "ENVELOPPE_DEPASSEE (budgets sommes au-dela du master 100)")}
