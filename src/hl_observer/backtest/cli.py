...
def run_backtest(
    ...
    with session_factory() as session:
        ...
        typer.secho(f"[*] Replaying {len(deltas)} events with the Unified Decision Engine...", fg="cyan")
        results = run_scenario_comparison(deltas, mids, base_cfg)

        typer.echo("\n" + "="*85)
        typer.secho(f"{'SCENARIO COMPARISON REPORT':^85}", bold=True)
        typer.echo("="*85)
        header = f"{'Scenario':<20} | {'Signals':<7} | {'Trades':<7} | {'Fees ($)':<8} | {'PnL ($)':<10} | {'ROI %':<8}"
        ...
        typer.echo("="*85)
        typer.secho("[!] All scenarios use the same pessimistic fill and fee models as live simulation.", fg="bright_black")
