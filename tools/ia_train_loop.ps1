...
$staleAfterSec = 180
if ($env:HYPERSMART_IA_MAX_SNAPSHOT_AGE_SEC) { try { $staleAfterSec = [Math]::Max(30, [int]$env:HYPERSMART_IA_MAX_SNAPSHOT_AGE_SEC) } catch { } }
...
while (-not (Test-StopRequested)) {
    ...
    if ($snap) {
        $ageSec = [Math]::Round(((Get-Date) - $snap.LastWriteTime).TotalSeconds)
        if ($ageSec -gt $staleAfterSec) {
            Write-IaTrainLog ("--- skip $(Get-Date -Format o) snapshot stale age=${ageSec}s max=${staleAfterSec}s path=$($snap.FullName) ---")
            Sleep-Interruptible -Seconds $interval
            continue
        }
        try {
...
Write-IaTrainLog ("--- stop requested $(Get-Date -Format o) ---")
