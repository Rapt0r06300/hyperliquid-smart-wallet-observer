...
REM   ASCII PUR, pas de pause -> tous_les_bugs.txt
REM ==================================================================================
echo ============ 1. TESTS DES NOUVEAUX MODULES ============ > tous_les_bugs.txt
python -m pytest -q tests\test_hyperliquid_fees.py tests\test_snapshot_capture.py tests\test_honest_metrics.py tests\test_execution_realism.py tests\test_zones_mortes_entree_mesuree.py >> tous_les_bugs.txt 2>&1

echo. >> tous_les_bugs.txt
echo ============ 2. BALAYAGE DIFFERENTIEL DU LOOKAHEAD ============ >> tous_les_bugs.txt
python tools\balayage_lookahead.py >> tous_les_bugs.txt 2>&1

echo. >> tous_les_bugs.txt
echo ============ 3. TRIAGE DES 91 ============ >> tous_les_bugs.txt
python tools\trier_h90_h180.py >> tous_les_bugs.txt 2>&1

echo. >> tous_les_bugs.txt
echo ============ 4. SUITE COMPLETE (la verite est sur Windows) ============ >> tous_les_bugs.txt
python -m pytest -q >> tous_les_bugs.txt 2>&1

echo. >> tous_les_bugs.txt
echo ============ 5. SECURITE ============ >> tous_les_bugs.txt
python -m hl_observer safety-audit >> tous_les_bugs.txt 2>&1
exit /b 0
