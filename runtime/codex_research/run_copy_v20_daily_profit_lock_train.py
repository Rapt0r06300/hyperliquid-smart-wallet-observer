...
def settle_variant(
    ...
    for metaorder in sorted(candidates, key=lambda row: (int(row["signal_ts_ms"]), str(row["metaorder_id"]))):
        ...
        trades.append({**actual, **common, "mechanism": "copy_v20_daily_profit_lock"})
        placebos.append({**placebo, **common, "mechanism": "copy_v20_daily_profit_lock_placebo"})
...
def main() -> None:
    ...
    report = {
        "schema_version": "hypersmart.copy_v20_daily_profit_lock_train.v1",
        "mechanism": "copy_v20_daily_profit_lock",
    ...
    output = ROOT / "runtime/reports/economic_campaigns/research_vnext/copy_vault_v20_daily_profit_lock_train.json"
