...
def settle_variant(
    ...
    *, capacity_fraction: float,
    ...
    for metaorder in sorted(candidates, key=lambda row: (int(row["signal_ts_ms"]), str(row["metaorder_id"]))):
        ...
        notional_usd = max(
            MIN_NOTIONAL_USD,
            min(MAX_NOTIONAL_USD, capacity_fraction * float(metaorder["reference_two_sided_capacity_usd"])),
        )
    ...
    for actual, placebo, metaorder in sorted(completed, key=lambda item: (int(item[0]["entry_ts_ms"]), str(item[0]["metaorder_id"]))):
        ...
        common = {
            ...
            "depth_sized_notional_usd": float(actual["notional_usd"]),
            "paper_read_only": True,
            "real_execution": False,
        }
        trades.append({**actual, **common, "mechanism": "copy_v18_depth_sized_alignment"})
        placebos.append({**placebo, **common, "mechanism": "copy_v18_depth_sized_alignment_placebo"})
...
def main() -> None:
    ...
    for minimum_alignment in SIGNED_DEPTH_IMBALANCE_MINIMUMS:
        ...
        for capacity_fraction in CAPACITY_FRACTIONS:
                    trades, placebos, replay_audit = settle_variant(
                        candidates, continuous_books, capacity_fraction=capacity_fraction,
                    )
    ...
    report = {
        "schema_version": "hypersmart.copy_v18_depth_sized_alignment_train.v1",
        "mechanism": "copy_v18_depth_sized_alignment",
    ...
    output = ROOT / "runtime/reports/economic_campaigns/research_vnext/copy_vault_v18_depth_sized_alignment_train.json"
