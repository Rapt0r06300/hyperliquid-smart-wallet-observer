...
FIRST_FILL_MIN_NOTIONALS_USD = (0.0,)
SIGNED_DEPTH_IMBALANCE_MINIMUMS = (0.0, 0.20, 0.50)
HORIZONS_MS = (1_800_000, 3_600_000)
NOTIONALS_USD = (600.0,)
VAULT_DAILY_LIMIT = 1
COIN_DAILY_LIMIT = 1
PRIOR_TRIALS = 412
...
def signed_reference_depth_imbalance(metaorder: dict, rows: list[dict]) -> float | None:
    if not rows:
        return None
    ...
    if total <= 0 or direction not in (-1, 1):
        return None
    return direction * (bid_usd - ask_usd) / total
...
def settle_variant(
    ...
    *, horizon_ms: int, notional_usd: float,
    ...
    for actual, placebo, metaorder in sorted(completed, key=lambda item: (int(item[0]["entry_ts_ms"]), str(item[0]["metaorder_id"]))):
        ...
        trades.append({**actual, **common, "mechanism": "copy_v17_balanced_alignment"})
        placebos.append({**placebo, **common, "mechanism": "copy_v17_balanced_alignment_placebo"})
...
def main() -> None:
    ...
    purge_ms = COPY_DELAY_MS + max(HORIZONS_MS) + MAX_TARGET_LAG_MS
    ...
    variants: list[dict] = []
    for minimum_size in FIRST_FILL_MIN_NOTIONALS_USD:
        for minimum_alignment in SIGNED_DEPTH_IMBALANCE_MINIMUMS:
            candidates = [
                row for row in train_adds
                if first_fill_notional_usd(row) >= minimum_size
                and float(row["signed_reference_depth_imbalance"]) >= minimum_alignment
            ]
            for horizon_ms in HORIZONS_MS:
                for notional_usd in NOTIONALS_USD:
                    trades, placebos, replay_audit = settle_variant(
                        candidates, continuous_books, horizon_ms=horizon_ms,
                    ...
                    variants.append({
                        "first_fill_min_notional_usd": minimum_size,
                        "signed_depth_imbalance_minimum": minimum_alignment,
                        "horizon_ms": horizon_ms, "notional_usd": notional_usd,
    ...
    report = {
        "schema_version": "hypersmart.copy_v17_balanced_alignment_train.v1",
        "mechanism": "copy_v17_balanced_alignment",
        ...
        "fixed_grid": {
            "first_fill_min_notionals_usd": FIRST_FILL_MIN_NOTIONALS_USD,
            "signed_depth_imbalance_minimums": SIGNED_DEPTH_IMBALANCE_MINIMUMS,
            "horizons_ms": HORIZONS_MS, "notionals_usd": NOTIONALS_USD,
    ...
    output = ROOT / "runtime/reports/economic_campaigns/research_vnext/copy_vault_v17_balanced_alignment_train.json"
