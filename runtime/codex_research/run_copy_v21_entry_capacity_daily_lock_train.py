...
from run_copy_v10_causal_risk_exits_train import apply_risk_exit  # noqa: E402

HORIZON_MS = 3_600_000
MIN_LEADER_FILL_NOTIONAL_USD = 100.0
CORE_MAX_NOTIONAL_USD = 600.0
...
def reference_depth_features(metaorder: dict, rows: list[dict]) -> tuple[float, float] | None:
    if not rows:
        return None
...
def settle_variant(
    ...
    for metaorder in sorted(candidates, key=lambda row: (int(row["signal_ts_ms"]), str(row["metaorder_id"]))):
        ...
        if coin_day[coin_key] >= COIN_DAILY_LIMIT:
            ...
            continue
        active_exit_times = [timestamp for timestamp in active_exit_times if timestamp > signal_ms]
        ...
        locked = realized_today >= DAILY_PROFIT_LOCK_USD
        if locked:
            notional_usd = PROBE_NOTIONAL_USD
            sizing_state = "POST_LOCK_NEW_VAULT_PROBE"
        else:
            notional_usd = CORE_MAX_NOTIONAL_USD
            sizing_state = "PRE_LOCK_DEPTH_CAPPED_CORE"
        if notional_usd < PROBE_NOTIONAL_USD:
            audit["SIZED_NOTIONAL_BELOW_MINIMUM"] += 1
            continue
        ...
        if actual is None or placebo is None:
            ...
            continue
        timestamps = [int(row["ts_ms"]) for row in rows]
        actual, actual_trigger = apply_risk_exit(
            actual, metaorder, rows, timestamps,
            stop_loss_bps=STOP_LOSS_BPS, take_profit_bps=TAKE_PROFIT_BPS,
        )
        placebo, placebo_trigger = apply_risk_exit(
            placebo, metaorder, rows, timestamps,
            stop_loss_bps=STOP_LOSS_BPS, take_profit_bps=TAKE_PROFIT_BPS,
        )
        ...
        trades.append({**actual, **common, "mechanism": "copy_v21_entry_capacity_daily_lock"})
        placebos.append({**placebo, **common, "mechanism": "copy_v21_entry_capacity_daily_lock_placebo"})
...
def main() -> None:
    ...
    candidates = [
        ...
        if float(row["signed_reference_depth_imbalance"]) >= 0.0
        and first_fill_notional_usd(row) >= MIN_LEADER_FILL_NOTIONAL_USD
    ]
    trades, placebos, replay_audit = settle_variant(candidates, continuous_books)
    ...
    variants: list[dict] = [{
        ...
        "minimum_leader_fill_notional_usd": MIN_LEADER_FILL_NOTIONAL_USD,
        "horizon_ms": HORIZON_MS,
        "core_maximum_notional_usd": CORE_MAX_NOTIONAL_USD,
    ...
    report = {
        "schema_version": "hypersmart.copy_v21_entry_capacity_daily_lock_train.v1",
    ...
    output = ROOT / "runtime/reports/economic_campaigns/research_vnext/copy_vault_v21_entry_capacity_daily_lock_train.json"
