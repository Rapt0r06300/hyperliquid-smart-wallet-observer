...
def test_registry_aggregates_statuses_without_promoting_discovery_only() -> None:
    ...
    assert market.instruments["kucoin"].status is AvailabilityStatus.DISCOVERY_ONLY
...
def test_gate_contract_and_sequenced_book_normalization() -> None:
    ...
    state.apply_ticker({"contract": "BTC_USDT", "mark_price": "100.7", "index_price": "100.6", "funding_rate": "0.0001", "volume_24h_base": "50", "total_size": "25"})
...
def test_bitget_instrument_and_book_normalization() -> None:
    payload = {"data": [{"symbol": "ETHUSDT", "baseCoin": "ETH", "quoteCoin": "USDT", "symbolStatus": "normal"}]}
    assert parse_bitget_instruments(payload) == [("ETH", "ETHUSDT")]
    state = BitgetMarketState(symbol="ETHUSDT", stale_after_ms=1_000)
    state.apply({"arg": {"channel": "books", "instId": "ETHUSDT"}, "action": "snapshot", "data": [{"ts": "1000", "seq": "10", "bids": [[10, 4]], "asks": [[11, 5]]}]}, receive_ts_ms=1_010)
    state.apply({"arg": {"channel": "ticker", "instId": "ETHUSDT"}, "data": [{"ts": "1010", "markPrice": "10.5", "indexPrice": "10.4", "fundingRate": "0.0002", "holdingAmount": "12"}]}, receive_ts_ms=1_020)
    snapshot = state.snapshot(now_ms=1_025)
...
def test_bitget_crossed_book_fails_closed() -> None:
    ...
    result = state.apply({"arg": {"channel": "books", "instId": "ETHUSDT"}, "action": "snapshot", "data": [{"ts": "1000", "seq": "1", "bids": [[12, 1]], "asks": [[11, 1]]}]}, receive_ts_ms=1_000)
...
def _record(data_type: HistoricalDataType, ts: int, **payload) -> HistoricalRecord:
    ...
    return HistoricalRecord(venue="binance", canonical_coin="BTC", exchange_symbol="BTCUSDT", data_type=data_type, exchange_timestamp=ts, receive_timestamp=receive_timestamp, source="local", provenance="fixture", normalized=True, sequence=sequence, event_id=event_id, payload=payload)


def test_local_jsonl_backfill_is_bounded_and_normalized(tmp_path) -> None:
    path = tmp_path / "trades.jsonl"
    path.write_text("\n".join(json.dumps(row) for row in [{"ts": 1, "price": 10}, {"ts": 2, "price": 11}, {"ts": 9, "price": 12}]), encoding="utf-8")
    hub = HistoricalBackfillHub()
    hub.register(LocalFileAdapter("local", path, field_map={"timestamp": "ts"}))
    result = hub.backfill(BackfillRequest(venue="binance", canonical_coin="BTC", exchange_symbol="BTCUSDT", data_type=HistoricalDataType.TRADES, start_timestamp=1, end_timestamp=3, limit=10, adapter="local"))
...
def test_tardis_import_is_optional_and_preserves_timestamps(tmp_path) -> None:
    ...
    path.write_text(json.dumps({"exchange": "okex-futures", "symbol": "BTC-USDT-SWAP", "type": "book_snapshot_5", "timestamp": "2026-01-01T00:00:00Z", "local_timestamp": "2026-01-01T00:00:00.010Z", "bids": [[100, 1]], "asks": [[101, 1]]}), encoding="utf-8")
    hub = HistoricalBackfillHub([TardisFileAdapter(path)])
    result = hub.backfill(BackfillRequest(venue="okx", canonical_coin="BTC", exchange_symbol="BTC-USDT-SWAP", data_type=HistoricalDataType.BBO, start_timestamp=0, end_timestamp=9_999_999_999_999, adapter="tardis"))
...
def test_broken_historical_source_does_not_break_batch() -> None:
    ...
    results = hub.backfill_many([BackfillRequest(venue="x", canonical_coin="BTC", exchange_symbol="BTC", data_type=HistoricalDataType.TRADES, start_timestamp=1, end_timestamp=2, adapter="broken")])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({HistoricalDataType.TRADES, HistoricalDataType.BBO, HistoricalDataType.FUNDING, HistoricalDataType.OPEN_INTEREST}, ReplayQuality.SILVER),
        ({HistoricalDataType.TRADES, HistoricalDataType.BBO, HistoricalDataType.ORDER_BOOK_L2, HistoricalDataType.FUNDING, HistoricalDataType.OPEN_INTEREST, HistoricalDataType.MARK_PRICE, HistoricalDataType.INDEX_PRICE, HistoricalDataType.LIQUIDATIONS}, ReplayQuality.GOLD),
    ],
)
def test_replay_quality_levels(types, expected) -> None:
    records = [_record(data_type, index + 1, sequence=index + 1, receive_timestamp=index + 2) for index, data_type in enumerate(types)]
    assert determine_replay_quality(records) is expected


def test_gold_requires_sequence_and_exchange_timestamp() -> None:
    types = {HistoricalDataType.TRADES, HistoricalDataType.BBO, HistoricalDataType.ORDER_BOOK_L2, HistoricalDataType.FUNDING, HistoricalDataType.OPEN_INTEREST, HistoricalDataType.MARK_PRICE, HistoricalDataType.INDEX_PRICE, HistoricalDataType.LIQUIDATIONS}
