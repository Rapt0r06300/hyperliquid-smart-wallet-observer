...
def test_no_derived_proof_claims_historical_literal_recovery() -> None:
    ...
    assert all(
        proof["provenance"] == "DERIVED_TECHNICAL_REQUIREMENT"
        for proof in result["proofs"]
    )


def test_current_progress_is_honest_465_with_copy_and_lead_lag_specific() -> None:
    result = evaluate(ROOT)
    assert result["source_contract_ok"] is True
    assert result["prior_1_320_assets_ok"] is True
    assert result["base_requirements_done"] == 29
    assert result["derived_proofs_done"] == 145
    assert result["technical_done"] == 465
    assert result["next_derived_id"] == 466
    ...
    assert result["evaluated_categories"] == ["COPY_VAULT", "LEAD_LAG"]
    assert result["category_progress"]["COPY_VAULT"]["ok"] is True
    assert result["category_progress"]["LEAD_LAG"]["ok"] is True


def test_future_categories_are_explicitly_incomplete_not_generic_file_green() -> None:
    result = evaluate(ROOT)
    future = [proof for proof in result["proofs"] if proof["id"] >= 466]
    assert future
    assert all(proof["ok"] is False for proof in future)
    assert all(
        proof["blocker"] == "CATEGORY_NOT_YET_SPECIFICALLY_VERIFIED"
        for proof in future
    )
    assert all(not proof["evidence"] for proof in future)


def test_copy_and_lead_lag_proofs_have_specific_hashed_evidence() -> None:
    result = evaluate(ROOT)
    proven = [proof for proof in result["proofs"] if 321 <= proof["id"] <= 465]
    assert len(proven) == 145
    assert all(proof["ok"] is True for proof in proven)
    for proof in proven:
...
def test_status_manifest_still_denies_literal_reconstruction() -> None:
    status = json.loads(
        (ROOT / "docs/PRE_RUN_775_CANONICAL_STATUS.json").read_text(
            encoding="utf-8"
        )
    )
