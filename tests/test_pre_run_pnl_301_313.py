...
def _proof(family: str, *, oos_net: float=2.2, forward_net: float=2.2):
    ...
    if family=="cross_venue_dislocation_v2": row["all_positions_two_leg_closed"]=True
    return row
...
def test_garde_775_refuse_explicitement_l_ancien_master_v6():
    ...
    result=validate_manifest(old); assert result["ok"] is False; assert "WRONG_ROADMAP_TOTAL" in result["issues"]; assert "CANONICAL_ANCHOR_MISMATCH:301" in result["issues"]
...
def test_done_775_exige_775_libelles_et_775_preuves():
    ...
    result=validate_manifest(fake_done); assert result["ok"] is False; assert "DONE_REQUIRES_775_LITERAL_LABELS" in result["issues"]; assert "DONE_REQUIRES_775_EXECUTABLE_PROOFS" in result["issues"]
