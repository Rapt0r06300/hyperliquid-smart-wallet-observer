...
from hl_observer.signals.depth_guard import depth_fill_guard, depth_guard
...
def test_depth_fill_guard_blocks_missed_or_slippy_book():
    ...
    assert code == "MISSED_FILL"
    assert result.fill_ratio < 0.85

    slippy, slippy_code, slippy_result = depth_fill_guard(
        side="LONG",
        needed_usd=100,
        mid_price=25,
        asks=((30.0, 10),),
    ...
    assert slippy is False
    assert slippy_code == "DEPTH_SLIPPAGE_TOO_HIGH"
