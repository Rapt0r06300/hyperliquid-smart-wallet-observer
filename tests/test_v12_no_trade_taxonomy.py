...
_V26_L1 = {  # vetos funding sain + edge stable (roadmap V26, repo 32 gajesh2007/funding-arb-bot)
    "FUNDING_SPIKE", "FUNDING_HISTORY_WARMUP", "EDGE_TRENDING_DOWN",
}
_V26_L3_L8 = {  # protections freqtrade, halt gradue, budget tier, forager (roadmap V26)
    "STOPLOSS_GUARD_ACTIVE", "MARKET_LOW_PROFIT_BLOCKED", "WINDOWED_DRAWDOWN_HALT",
    "GRADED_HALT_AMBER", "GRADED_HALT_RED", "COST_BUDGET_EXCEEDED", "MARKET_QUALITY_LOW",
}


def test_taxonomy_covers_both_doc_blocks_exactly():
    expected = _V10_6 | _V11_11 | _V12_CLUSTER | _V26_L1 | _V26_L3_L8
    actual = set(all_codes())
    assert actual == expected, "missing=%s extra=%s" % (expected - actual, actual - expected)
    assert len(all_codes()) == len(_V10_6) + len(_V11_11) + len(_V12_CLUSTER) + len(_V26_L1) + len(_V26_L3_L8) == 70
