...
def test_source_loss_terminal_est_honnete_et_progression_technique_non_faussee() -> None:
    ...
    assert manifest["technical_completion_done"] == 465
...
def test_exigences_thematiques_sont_scellees_par_sha256() -> None:
    manifest = _manifest()
    payload = THEMATIC_PATH.read_bytes()
...
def test_source_loss_refuse_un_faux_jeu_de_labels_ou_preuves() -> None:
    manifest = _manifest()
    manifest["labels"] = ["inventé"] * ROADMAP_TOTAL
    manifest["proofs"] = {
        str(i): f"preuve-{i}" for i in range(1, ROADMAP_TOTAL + 1)
    }
    result = validate_manifest(manifest)
    assert result["ok"] is False
