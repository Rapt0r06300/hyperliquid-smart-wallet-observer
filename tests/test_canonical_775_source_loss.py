...
def test_source_loss_remains_honest_at_progress_or_technical_done() -> None:
...
def test_exigences_thematiques_sont_scellees_par_sha256() -> None:
    ...
    assert manifest["thematic_requirements_path"] == THEMATIC_REQUIREMENTS_PATH
    assert digest == manifest["thematic_requirements_sha256"]
...
def test_source_loss_ne_peut_jamais_devenir_un_faux_done_litteral() -> None:
...
def test_source_loss_refuse_un_faux_jeu_de_labels_ou_preuves() -> None:
    manifest = _manifest(); manifest["labels"] = ["inventé"] * ROADMAP_TOTAL
    manifest["proofs"] = {str(i): f"preuve-{i}" for i in range(1, ROADMAP_TOTAL + 1)}
