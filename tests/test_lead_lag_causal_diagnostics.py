...
def _book(ts_ms: int, *, ready: bool = True, reasons=(), gap_count: int = 0, reconnect_count: int = 0) -> dict[str, object]:
    return {"ts_ms": ts_ms, "data_gate_ready": ready, "quality_reasons": list(reasons), "gap_count": gap_count, "reconnect_count": reconnect_count}


def _meta(trigger: int, *, stopped_reason: str = "COMPLETED", **extra) -> dict:
    return {"windows": [{"start_ms": trigger - 1000, "end_ms": trigger + 15000}], "per_window": [{"stopped_reason": stopped_reason, **extra}]}
...
def test_classifies_timely_quality_ready_book_as_executable() -> None:
    ...
    result = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": [_book(trigger + 125)]}, _meta(trigger))
...
def test_timely_book_rejected_by_quality_is_not_called_executable() -> None:
    trigger = 1_800_000_000_000
    result = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": [_book(trigger + 100, ready=False, reasons=["STALE_BOOK"])]}, _meta(trigger))
    row = result["events"][0]
...
def test_partial_local_scan_is_inconclusive_not_a_fake_collector_gap() -> None:
    trigger = 1_800_000_000_000
    result = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": [_book(trigger + 2_295)]}, _meta(trigger, stopped_reason="TIME_BUDGET_REACHED"))
    row = result["events"][0]
...
def test_late_clean_book_is_distinguished_from_no_later_book() -> None:
    trigger = 1_800_000_000_000
    late = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": [_book(trigger + 4_715)]}, _meta(trigger))
...
def test_explicit_gap_counter_remains_real_feed_gap_evidence() -> None:
    trigger = 1_800_000_000_000
    result = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": [_book(trigger + 2_295, gap_count=3)]}, _meta(trigger))
    row = result["events"][0]
...
def test_window_gap_counter_is_gap_evidence_but_timeout_is_not() -> None:
    trigger = 1_800_000_000_000
    result = diagnose_causal_book_coverage([_shock(trigger)], {"ETH": []}, _meta(trigger, gap_count=2))
...
def test_delay_distribution_reports_observed_2295_and_4715_ms_cases() -> None:
    ...
    second = first + 60_000
    result = diagnose_causal_book_coverage([_shock(first), _shock(second, -8.3)], {"ETH": [_book(first + 2_295), _book(second + 4_715)]}, {"windows": [{"start_ms": first - 1000, "end_ms": first + 15000}, {"start_ms": second - 1000, "end_ms": second + 15000}], "per_window": [{"stopped_reason": "COMPLETED"}, {"stopped_reason": "COMPLETED"}]})
