...
def test_canonical_status_is_honest_in_progress_465() -> None:
    manifest = _manifest()
...
def test_canonical_status_matches_specific_executable_progress_gate() -> None:
    ...
    assert gate["technical_done"] == manifest["technical_completion_done"] == 465
    assert gate["derived_proofs_done"] == manifest["derived_technical_controls"]["done"] == 145
    assert gate["base_requirements_done"] == manifest["derived_technical_controls"]["base_requirements_done"] == 29
    assert manifest["derived_technical_controls"]["specifically_verified_categories"] == [
        "COPY_VAULT",
        "LEAD_LAG",
    ]
    assert manifest["derived_technical_controls"]["next_unverified_id"] == 466
...
def test_superseded_generic_file_completion_is_recorded_as_withdrawn() -> None:
    ...
    assert retired["previous_claim"] == 775
