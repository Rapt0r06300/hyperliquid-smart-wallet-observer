...
def _manifest() -> dict[str, object]: return json.loads(STATUS_PATH.read_text(encoding="utf-8"))

def test_canonical_status_is_honest_in_progress_395() -> None:
...
def test_canonical_status_matches_specific_executable_progress_gate() -> None:
    ...
    assert gate["technical_done"] == manifest["technical_completion_done"] == 395
    assert gate["derived_proofs_done"] == manifest["derived_technical_controls"]["done"] == 75
    assert gate["base_requirements_done"] == manifest["derived_technical_controls"]["base_requirements_done"] == 15
