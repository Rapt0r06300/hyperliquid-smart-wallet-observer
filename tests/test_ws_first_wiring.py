...
def test_off_is_noop(monkeypatch):
    ...
    assert p.all_mids and p.user_fills and p.open_orders     # rien changé
...
def test_on_drops_ws_covered(monkeypatch):
    ...
    p = _maybe_apply_ws_first(_p())
    assert p.all_mids is False and p.user_fills is False     # WS couvre -> REST coupé
    assert p.open_orders is True                              # non couvert -> gardé
