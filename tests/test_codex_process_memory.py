...
def test_negative_memory_veto_positive_boost_and_retest_override():
    ...
    candidate = {"family": "lead_lag", "mechanism_signature": "binance-bbo-leads-hl", "context": ["high-liquidity"]}
    effect = candidate_memory_effect(candidate, failures)
    assert effect["veto"] is True
    successes = [_record(record_id="PM-S", outcome="SUCCESS", confidence=0.9, success_evidence="positive OOS", failure_reason=None)]
