...
def test_plan_utilise_le_code_principal_et_les_donnees_du_workspace(tmp_path: Path) -> None:
    ...
    assert "walk_forward" in {stage.key for stage in plan}
    assert "anti_overfit" in {stage.key for stage in plan}


def test_runner_ecrit_provenance_de_suite_avec_un_faux_executeur(tmp_path: Path) -> None:
    project = tmp_path / "project"
    data = tmp_path / "dataset"
