...
def test_adaptive_paper_sizing_blocks_marginal_edge_after_three_losses():
    recent_events = [
        {"status": "LOCAL_REPLAY", "bot_replay_action": "PAPER_CLOSE_REPLAYED", "estimated_net_pnl_usdc": -0.5},
        {"status": "LOCAL_REPLAY", "bot_replay_action": "PAPER_CLOSE_REPLAYED", "estimated_net_pnl_usdc": -0.4},
        {"status": "LOCAL_REPLAY", "bot_replay_action": "PAPER_CLOSE_REPLAYED", "estimated_net_pnl_usdc": -0.3},
    ]
    ...
    assert sizing.final_margin_usdt == 0.0
    assert sizing.reason == "ADAPTIVE_SIZE_BLOCKED_AFTER_LOSS_STREAK"


def test_sltp_stop_loss_waits_for_min_hold_unless_catastrophic(monkeypatch):
    monkeypatch.setenv("HYPERSMART_SLTP_STOP_MIN_HOLD_MS", "45000")
    monkeypatch.setenv("HYPERSMART_SLTP_CATASTROPHIC_STOP_BPS", "120")
    cfg = SLTPConfig(stop_loss_bps=30.0, take_profit_bps=99999.0, trailing_stop_bps=None)
    positions = {
        ("w", "HYPE", "LONG"): {
            "size": 1.0,
            "avg_price": 100.0,
    ...
    closed = apply_sltp_exits(positions, ledger, {"HYPE": 99.6}, now_ms=5_000, config=cfg)

    assert closed == []
    assert ("w", "HYPE", "LONG") in positions
    assert positions[("w", "HYPE", "LONG")]["last_sltp_hold_reason"] == "STOP_LOSS_MIN_HOLD_NOT_REACHED"

    closed = apply_sltp_exits(positions, ledger, {"HYPE": 98.7}, now_ms=6_000, config=cfg)

    assert closed and closed[0]["reason"] == "STOP_LOSS"
    assert ("w", "HYPE", "LONG") not in positions
    assert ledger[-1]["sltp_catastrophic_stop_bps"] == 120.0
