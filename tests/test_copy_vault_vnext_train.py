...
def _row(*, ts: int, vault: str, segment: str = "train", net: float = 1.0) -> dict:
    return {
        "trade_id": f"{segment}-{vault}-{ts}",
        "vault": vault,
        "coin": "ETH",
        "direction": 1,
        "signal_ts_ms": ts,
...
def test_copy_vnext_selection_ignore_totalement_oos_et_exige_consensus_prior_only() -> None:
    ...
    for day_index in range(4):
        ...
        rows.extend(
            [
                _row(ts=base + 1_000, vault="0xa"),
                _row(ts=base + 2_000, vault="0xb"),
                _row(ts=base + 3_000, vault="0xc"),
            ]
        )
    # A gigantic heldout winner must not alter TRAIN selection or its net.
    rows.append(_row(ts=99_999 * day, vault="0xdead", segment="oos", net=1_000_000.0))
    report = {
