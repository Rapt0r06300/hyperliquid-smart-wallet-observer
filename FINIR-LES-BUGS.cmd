...
REM   ASCII PUR, pas de pause -> les_bugs.txt
REM ==================================================================================
echo ============ 1. TESTS DES NOUVEAUX MODULES ============ > les_bugs.txt
python -m pytest -q tests\test_hyperliquid_fees.py tests\test_snapshot_capture.py tests\test_honest_metrics.py >> les_bugs.txt 2>&1

echo. >> les_bugs.txt
echo ============ 2. BALAYAGE DIFFERENTIEL DU LOOKAHEAD ============ >> les_bugs.txt
echo    (il se controle sur garch11_variance : s'il ne le retrouve pas, il se TAIT) >> les_bugs.txt
python tools\balayage_lookahead.py >> les_bugs.txt 2>&1

echo. >> les_bugs.txt
echo ============ 3. TRIAGE DES 91 ============ >> les_bugs.txt
python tools\trier_h90_h180.py >> les_bugs.txt 2>&1

echo. >> les_bugs.txt
echo ============ 4. SUITE COMPLETE (la verite est sur Windows) ============ >> les_bugs.txt
python -m pytest -q >> les_bugs.txt 2>&1

echo. >> les_bugs.txt
echo ============ 5. SECURITE ============ >> les_bugs.txt
python -m hl_observer safety-audit >> les_bugs.txt 2>&1
exit /b 0
