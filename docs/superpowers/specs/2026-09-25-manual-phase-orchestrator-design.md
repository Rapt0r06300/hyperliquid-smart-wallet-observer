...
1440. portfolio drawdown evaluation rejects non-finite/invalid peak equity, current equity and threshold inputs before arithmetic and cannot return OK/zero drawdown for NaN/Infinity;
1441. deterministic NaN/Infinity fixtures prove portfolio drawdown kill-switch state fails closed rather than being neutralized by max/clamp behavior;
1442. volatility-target sizing rejects non-finite or invalid target/asset volatility, base percentage and cap values and can never transform NaN volatility into the maximum cap;
1443. risk-per-trade sizing rejects non-finite equity/risk/stop inputs, negative risk percent and out-of-policy risk percentages before producing a notional;
1444. all risk helper outputs consumed by authoritative sizing are finite, non-negative and policy-bounded, with typed invalid/no-trade states instead of NaN/Infinity propagation;
1445. all blocker-classified weaknesses 241-243 remain implementation blockers until non-finite/domain/fail-closed risk-helper regression tests prove closure.
