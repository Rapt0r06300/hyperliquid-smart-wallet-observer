...
281. **Objective-loop checkpoint corruption/write failure is fail-open for the trial ledger.** `_lire_etat` turns any read/JSON failure into a fresh empty `{"essais": []}`, while `_ecrire_etat` logs an `OSError` and continues. After a crash/restart this can erase the historical trial count and rerun configurations without an explicit `STATE_LOST` boundary, undermining multiplicity/Deflated-Sharpe accounting and the claim that every trial is durably remembered.
282. **The module claiming backtest/live risk parity does not consume the live resolved risk configuration.** `backtesting/backtest_live_parity.py` declares independent constants such as `MAX_POSITIONS_PAR_COIN=1`, `MAX_POSITIONS_TOTAL=5`, `MAX_EXPOSITION_NETTE=1.0` and `MAX_PART_PAR_COIN=0.60`, then calls them “les MÊMES limites que le live”. It does not bind those limits to the `Settings`/authoritative risk pipeline used by the evaluated run. Configuration drift can therefore leave the parity diagnostic green while live and replay enforce different admissible exposure.
283. **The supposedly immutable backtest baseline is not a complete proof identity.** `backtest_live_parity.empreinte` hashes only caller-supplied data/config, truncates SHA-256 to 16 hexadecimal characters and uses `default=str`; it excludes code/tree SHA, dependency profile, fee/rule registry and execution-model identity. The same baseline can remain “valid” after economically relevant implementation changes, and its short/non-canonical fingerprint is weaker than the full-content proof identities required elsewhere.
284. **Durable deduplication forgets compacted event ids for future duplicate checks.** `realtime/durable_dedup.py::compact` archives the oldest rows and deletes them from `seen_events`, while `check_and_mark` and `contains` query only the live SQLite table. After compaction, a redelivered/backfilled event whose id exists only in an archive is accepted as new and can be applied twice.
285. **Dedup archives are durable files but not part of a verifiable membership authority.** Archive JSONL is fsynced, but there is no archive index/Merkle/hash-chain membership check used by duplicate decisions and no startup validation that archived ids remain queryable. The implementation therefore cannot claim restart/lifetime exactly-once semantics beyond the bounded live window.
286. **RawSpool is not crash-durable at the point it claims a raw frame is written.** `realtime/raw_spool.py::RawSpool.ecrire` writes through a buffered text handle, flushes only every configured batch and never fsyncs the file/directory. A process/power crash can lose recent frames after the socket handler considered them spooled, violating “raw before parse” durability.
287. **RawSpool replay silently drops corrupt evidence.** `RawSpool.relire` catches every exception for an unreadable line and simply continues without a rejection counter, byte/line position or contamination status. A damaged raw spool can therefore replay as a shorter apparently clean stream.
...
300. **Live userFills position reconstruction accepts non-finite prices/sizes and can poison Copy-Vault state.** `collection/userfills_live.py::positions_depuis_snapshot`, `appliquer_fill` and `parser_message_userfills` rely on plain `float()` conversions without `math.isfinite` or strict positive-price/size domains. NaN/Infinity can survive ordinary comparisons, enter `szi` / `entryPx` and propagate into reconstructed leader state instead of being quarantined.
301. **A malformed user fill can still be marked paper-eligible when critical economics/time are missing.** `wallets/position_delta_engine.py::confidence_for_fill` merely subtracts 0.15 for missing price and 0.10 for missing timestamp. With a valid side/startPosition, the resulting scores remain above the current `is_paper_eligible >= 0.7` threshold. Non-finite price/size/startPosition also count as present because `safe_float` does not reject them.
302. **The live userFills freshness gate accepts future and timestamp-less fills.** `wallets/user_fills_live.py::store_user_fills_live_result` rejects only when `received_at_ms - fill_time_ms > max_age`. A future exchange timestamp yields negative age and passes as fresh; a missing timestamp bypasses the age check entirely and proceeds to storage/delta construction.
303. **Collection source health fabricates event freshness when event time is missing and clamps future events to age zero.** `storage/repositories.py::update_source_health` substitutes `current_ms` for a successful call without an event timestamp and computes `max(0, current_ms-event_ms)`. A heartbeat/API success with no market event can therefore become a fresh market event, while future-dated evidence is also classified FRESH.
304. **Fill deduplication is payload-hash based rather than native-event-identity based.** `CollectionRepository.store_fills` hashes the entire wallet+fill payload and uses that hash as the duplicate authority even though Hyperliquid provides native fill/trade lineage such as time/coin/`tid` plus order/hash fields. The same native fill with harmless representation/enrichment differences can be stored twice, while conflicting payloads for one native identity are not surfaced as an identity conflict.
...
374. **The economic collection/resume plan trusts campaign status strings as proof authority.** `simulation/economic_collection_plan.py` classifies a family as PROVEN from `campaign.objective_status == "ATTEINT"` and sets global `goal_complete` when all three strings equal ATTEINT. It does not revalidate the canonical daily certificate, proof hashes or final gate DAG at that boundary; a stale/tampered/legacy status can therefore suppress collection and look globally complete.
375. **The collection plan still encodes cumulative +4 USD semantics and weak temporal receipts.** Its published target is `target_net_usd_per_family=4.0` and promotion prose says “>= +4 USD reconciled net”, not +4 USD per complete UTC day. `_temporal_progress` accepts one positive liquidatable OOS/forward segment from sample-count/net/Boolean flags without binding the daily-calendar/certificate receipt required by the canonical objective.
376. **The Lead-Lag campaign adapter converts missing economics/placebo evidence into numeric zero.** `simulation/lead_lag_campaign_adapter.campaign_from_replay` aggregates each missing segment economic field with `segment.get(key) or 0.0` and defaults missing placebo net to `0.0`. A positive candidate can therefore report `placebos.beaten=True` when no placebo net was actually measured, and incomplete segment economics can be hidden in totals.
377. **The Lead-Lag campaign adapter asserts anti-lookahead rather than proving it.** It writes `oos.no_lookahead=True` and signal metadata `no_lookahead=True` by construction. Those Booleans are not themselves a temporal-overlap/purge receipt, so a downstream objective must not treat them as independent proof.
378. **Economic identity falls back to ambiguous COIN:SIDE for authoritative position/episode keys.** `simulation/economic_identity.EconomicIdentity.position_key` uses `COIN:SIDE` when `position_id` is absent and `episode_key` falls back again to that position key. Concurrent or successive exposures on the same instrument/side can therefore collapse onto one identity at exactly the boundary intended to prevent ambiguous accounting.
...
Risk gates consume only finite, domain-valid values and fail closed before any sizing or halt decision.

- peak/current equity, drawdown threshold, volatility, target volatility, risk percentage, stop distance, base size and caps are finite and inside explicit configured domains;
- invalid/non-finite current or peak equity produces `RISK_STATE_INVALID`/halt, never a zero drawdown;
- drawdown computation never uses clamping that can convert NaN/Infinity or an invalid peak into a healthy value;
- volatility-target sizing with missing/non-finite volatility returns a typed no-trade/block state, never the maximum size cap;
- `risk_pct` is finite, non-negative and bounded by canonical policy before notional arithmetic;
- risk-per-trade notional is finite, non-negative and bounded before it can become requested/approved exposure;
- mutation tests inject NaN, +Infinity, -Infinity, zero/negative peak equity, negative risk percent and absurdly large risk percent into each risk helper and prove no new exposure is admitted and no kill-switch is suppressed.
...
1362. PaperLedger reduce/close verifies explicit position id, stored coin and stored side before PnL/mutation; caller side cannot invert the stored position's PnL equation;
1363. PaperLedger opening/addition cannot create an unfinanceable state: available collateral/buying power/margin is checked before mutation under the canonical 100-USD paper-capital policy;
1364. PaperEngine `_positions`, realized PnL, PaperLedger cash/positions/events/capital and liquidity reservations commit atomically or roll back together on any late validation/sealing error;
1365. deterministic fault injection after each paper-mutation substep proves failed operations leave byte-for-byte equivalent pre-call economic state;
1366. `reconcile_pnl` rejects NaN/±Infinity/non-finite accounting and can never return ok=true when any required operand or diff is non-finite;
1367. PaperLedger mark-to-market rejects non-finite/non-positive marks before updating stored marks, unrealized PnL, high-water equity or event state;
1368. invalid/missing marks are typed as stale/unmeasurable rather than silently inheriting previous/entry price in proof-facing equity;
1369. `ROI_avg_margin_locked` either becomes causal time-weighted average capital-at-risk or is renamed/typed so an equal-weight state-change average cannot masquerade as time-average capital usage;
1370. maker execution uses a shared consumption/reservation ledger for queue/trade-through evidence, preventing two paper intents from claiming the same public executed quantity;
1371. maker fill receipts bind the exact consuming public event identities/timestamps and conserved quantities used to move through queue-ahead and fill the modeled order;
1372. all blocker-classified weaknesses 182-188 from the 2026-09-26 paper-accounting continuation audit remain implementation blockers until transactional-state, capital-financeability, non-finite-reconciliation and maker-conservation tests prove closure.
1373. realtime replay is explicitly EVENT_REPLAY_ONLY unless it re-executes the canonical strategy/risk/decision stack from immutable historical inputs;
1374. semantic replay binds exact visible market/leader/config/risk/clock inputs and compares recomputed decision, reason, approved size and PaperIntent against the historical receipt;
1375. MarketTruthPipeline deduplicates canonical events before both persistence and TruthChain consumption, and duplicate PUBLIC_TRADE_BATCH fixtures cannot change maker matched volume/fill state;
1376. CanonicalEventWriter exactly-once identity survives process restart by reloading/checkpointing durable event ids or an equivalent complete dedupe index;
1377. replaying the same canonical Market-Truth event set before/after restart yields identical event count, fill result and proof/evidence hash;
1378. market_truth evaluate_research_candidate cannot emit PEPITE/positive proof when execution/reconciliation evidence is empty, incomplete or not one-to-one bound to counted backtest/forward episodes;
1379. feed_quality_score is finite and constrained to its canonical domain, and positive Market-Truth validation consumes the exact hash-bound data-gate receipt rather than Python truthiness;
1380. Market-Truth forward PnL parsing accounts for every input row and rejects/quarantines parse failures and NaN/±Infinity instead of dropping them from the sample;
1381. all blocker-classified weaknesses 189-194 from the 2026-09-26 semantic-replay/Market-Truth audit remain implementation blockers until deterministic duplicate/restart/parity/evidence-binding tests prove closure.
1382. official archive import exposes accepted/rejected row counts and malformed required rows cannot disappear from proof evidence;
1383. official archive price/size/timestamp fields reject NaN/±Infinity before TickEnvelope creation;
1384. archive max-events bounds produce an explicit TRUNCATED/non-certifiable state with retained/source range evidence instead of a silently shortened day;
1385. build_selection_plan filters records through the requested DatasetSuite and its digest/assets/counts match the suite predicate exactly;
1386. merged economic-input reuse requires immutable content identity for every source; same-size/same-mtime mutation invalidates the cache;
1387. stat/open/read failure of a required economic source is represented in the merge manifest and blocks complete/certifying source status;
1388. V2 discovery exposes inspected/skipped/truncated release counts and never interprets an unscanned history frontier as confirmed absence;
1389. malformed or inaccessible V2 run releases produce typed visible discovery errors while remaining excluded from SAFE inputs;
1390. attach_reconciliation verifies an immutable same-shard/same-run/same-window receipt before MATCHED can alter quality status;
1391. V2 RTT/clock/skew statistics reject non-finite observations and threshold checks cannot be bypassed by NaN;
1392. an explicit zero-event/zero-fill batch contributes zero to trade_count and regression fixtures prevent minimum-sample inflation;
1393. V2 manifest construction counts non-object/schema-invalid JSON rows and SAFE cannot be achieved by silently dropping them;
1394. all blocker-classified weaknesses 195-206 remain implementation blockers until deterministic archive-truncation, content-cache, release-frontier, reconciliation-substitution and V2 conservation tests prove closure.
1395. canonical market-event identity is derived from stable native/source identity and remains unchanged when the same venue event is redelivered with different local receive/write timestamps;
1396. same-millisecond market events use authoritative venue sequence or connection-scoped monotonic ordering where available and never lexical/hash order as a causal fallback;
1397. PaperLedger duplicate/idempotency identity cannot be converted into a fresh event id; replaying the same economic action has exactly-once state, PnL and fee effect;
1398. ledger read state distinguishes ABSENT from valid intentionally EMPTY and CORRUPT, and only an explicitly initialized empty ledger may be healthy with zero events;
1399. Market-Truth canonicalization accepts data_gate_ready only as a strict boolean and feed_quality_score only as a finite value inside the canonical allowed domain;
1400. all blocker-classified weaknesses 207-211 from the 2026-09-26 identity/idempotence audit remain implementation blockers until native-redelivery, same-ms-order, duplicate-action, missing-ledger and malformed-quality regression tests prove closure.
1401. TruthChain/economic chain identity binds a cryptographic digest of the complete ordered canonical market-event set plus execution-model/config/reconciliation identities used to determine the result;
1402. mutating, inserting, deleting or reordering any causally consumed market event changes the proof identity or deterministically invalidates the receipt;
1403. Market-Truth EvidenceWriter provides restart-durable sequence/integrity/duplicate detection equivalent to proof-critical journals, and edited/reordered/truncated evidence is detected;
1404. PaperLedger funding requires a matching open-position exposure and an immutable native/canonical settlement receipt containing coin, side, settlement time, rate/notional basis and unique event identity;
...
1432. Lead-Lag certifying episodes enforce a frozen maximum entry-observation/decision delay independent of the tested alpha horizon;
1433. an entry quote arriving after the frozen executable-entry latency bound cannot set liquidatable_net=true even when it arrives before the nominal exit horizon;
1434. every public trade that advances maker queue position has stable exactly-once native/canonical identity and required causal exchange/event timestamp evidence;
1435. duplicate redelivery of one public trade leaves Lead-Lag queue position, fill status and PnL unchanged;
1436. missing exchange/event time on a queue-consuming public trade blocks certifying FIFO evidence whenever the clock contract requires that domain;
...
1471. portfolio drawdown evaluation rejects non-finite/invalid peak equity, current equity and threshold inputs before arithmetic and cannot return OK/zero drawdown for NaN/Infinity;
1472. deterministic NaN/Infinity fixtures prove portfolio drawdown kill-switch state fails closed rather than being neutralized by max/clamp behavior;
1473. volatility-target sizing rejects non-finite or invalid target/asset volatility, base percentage and cap values and can never transform NaN volatility into the maximum cap;
...
1502. objective-loop promotion consumes a strict typed Boolean/receipt; strings, integers, mappings and other truthy values are INVALID_GATE_RESULT and cannot promote;
1503. objective-loop checkpoint read corruption or write failure produces a durable STATE_LOST/CHECKPOINT_UNTRUSTED condition and cannot silently reset the trial ledger;
1504. multiple-testing/trial counts remain monotone and restart-durable across crashes/resumes, including failed/error trials, so DSR/PBO/Reality-Check inputs cannot undercount the search actually performed;
1505. backtest/live parity derives position/exposure/concentration limits from the same resolved authoritative risk configuration/receipt as the run under test and rejects any unbound copied constants as diagnostic-only;
1506. parity tests mutate each authoritative live risk limit independently and prove the replay/parity result changes or fails closed rather than remaining pinned to stale constants;
1507. immutable baseline identity uses full cryptographic content identity and binds code/tree, dependencies, resolved config, fee/rule registry, execution model and exact data manifest; a code-only economic change invalidates comparison even when caller data/config objects are unchanged;
1508. all blocker-classified weaknesses 279-283 remain implementation blockers until stale-resume, malformed-gate, checkpoint-loss, risk-config-drift and baseline-identity regression tests prove closure.
1509. DurableEventDedup duplicate decisions remain correct after compaction; ids present only in immutable archives are still recognized as previously seen for the applicable proof horizon;
1510. dedup compaction archive publication, membership-index publication and live-row deletion are crash-idempotent and startup-validatable;
1511. corruption/removal/substitution of a required dedup archive/index yields DEDUP_EVIDENCE_CORRUPT and blocks exactly-once certification;
1512. RawSpool distinguishes buffered write from durable spool commit and crash-injection proves every acknowledged durable raw frame survives restart;
1513. RawSpool replay reports malformed/unreadable lines with exact counts/locations and cannot silently shorten proof evidence;
1514. stable_item_id is invariant to all local receipt/write/monotonic/connection/frame metadata aliases and depends only on canonical native event semantics plus source/channel identity;
1515. canonical identity regression fixtures mutate local timestamps and transport coordinates without changing the stable id, while mutating native economic identity changes it;
1516. bounded latency-journal summaries report total observations, retained observations, evictions and retained time coverage before they may support run-level latency claims;
1517. all blocker-classified weaknesses 284-289 remain implementation blockers until compaction-redelivery, crash-durability, corrupt-spool and identity-alias regression tests prove closure.
...
1531. userFills snapshot/fill state reconstruction rejects NaN/Infinity, non-positive prices/sizes and invalid start positions before any position mutation;
1532. a fill missing finite positive price, finite positive size, valid direction/native identity or positive exchange timestamp is never paper-eligible regardless of confidence heuristics;
1533. live userFills freshness requires a valid exchange timestamp and rejects materially future-dated fills under the configured clock-uncertainty policy;
1534. successful heartbeat/API receipt without a market-event timestamp updates connection health separately and cannot fabricate last_event_at_ms/current market freshness;
1535. source-health age computation rejects future/non-finite event time and never clamps causal violations to zero age/FRESH;
1536. fill dedup authority uses canonical native-event identity (including instrument/time/tid lineage as applicable) and detects same-identity conflicting payloads as contamination rather than storing both;
1537. userFills decoding/parsing conserves received frame/item cardinality and reports decode failures, malformed rows, rejected identities and reasons before proof use;
...
1638. economic collection/resume state derives PROVEN/goal_complete only from current canonical daily certificate receipts whose artifact/code/config/data hashes revalidate; objective_status text alone has no authority;
1639. collection-plan target metadata and promotion prose explicitly use +4 USD per complete UTC day per family and bind the same daily-policy schema/hash as final certification;
1640. collection-plan OOS/forward progress consumes canonical temporal/daily receipts rather than accepting free-standing no_lookahead/post_freeze/liquidatable booleans as sufficient proof;
1641. Lead-Lag campaign aggregation treats a missing mandatory segment economic component as UNMEASURABLE and cannot replace it with zero;
1642. missing placebo evidence is MISSING/UNMEASURABLE and can never become a zero-PnL placebo or placebo_beaten=True;
1643. Lead-Lag no-lookahead status is derived from a hash-bound temporal/purge/episode-containment receipt and is never hard-coded True in proof-facing campaign evidence;
1644. canonical position/episode accounting requires position_id/episode_id (or an explicitly unique equivalent lineage id); COIN:SIDE fallback is diagnostic-only and cannot authorize proof accounting;
1645. identity stamping compares every overlapping field and emits IDENTITY_CONFLICT on disagreement rather than silently retaining either value;
