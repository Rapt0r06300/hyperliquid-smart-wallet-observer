...
| P0-2 | **Une seule couche faisant autorité** — dashboard, rapport, ledger, laboratoire. | `PARTIALLY_DONE` | le réalisé vient du ledger pour tous ✅ ; **21/07 : la COURBE aussi** — `ui/courbe_equity` la reconstruit depuis le ledger et un test vérifie que son dernier point == `stable_net_pnl` du bandeau (mesuré : −5,639395 des deux côtés). Restant : le MtM de base reste recomposé côté endpoint |
| P0-2bis | **Métagraphe refait entièrement** (Flo, 21/07 : « il est complètement éclaté ») | `DONE_VERIFIED` | cause : la courbe servait 600 points **tous à 1 000,00 / pnl 0,0** (pile copy éteinte depuis le 11/07) avec le net carry greffé sur le **dernier point** → 599 points plats + une falaise verticale. Corrigé : série issue du ledger (45 points, amplitude 6,08 $), point vivant construit sur **la même** série, cas dégénérés déclarés au lieu d'être masqués par `rng=(hi-lo)||1`. 19 tests. Plus gros saut : **5,9 %** de l'amplitude contre ~100 % |
